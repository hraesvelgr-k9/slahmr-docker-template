...
GPU_IDS="${GPU_IDS:-0}"
CFG_PATH="${VIDEO_CFG:-/workspace/slahmr/slahmr/confs/data/video.yaml}"

SRC_DIR="$(dirname "${SRC_VIDEO}")"
VIDEO_DIR_BASENAME="$(basename "${SRC_DIR}")"
VIDEO_BASENAME="$(basename "${SRC_VIDEO}")"
...
echo "[INFO] config path  : ${CFG_PATH}"
...
echo "[STEP 2] Write Hydra video config"
cat > "${CFG_PATH}" <<EOCFG
type: video
split: ${DATA_SPLIT}
root: ${DATA_ROOT}
video_dir: ${VIDEO_DIR_BASENAME}
seq: ${SEQ_NAME}
ext: ${VIDEO_EXT}
...
echo "[INFO] next    : /workspace/slahmr/scripts/run_video_demo.sh"